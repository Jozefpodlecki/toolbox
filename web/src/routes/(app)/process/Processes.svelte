<script lang="ts">
    ...
    type ProcessResult = 
        | { type: "list"; data: Array<any> }
        | { type: "hierarchy"; data: Array<any> };
    ...
    async function onLoad() {
        ...
        const processes = await getProcesses(args);
        pageState.result = processes;
    }
...
{#if pageState.result}
    {#if pageState.result.type === "list"}
        {#each pageState.result.data as proc (proc.id)}
            <div class="pl-2 border-b border-gray-700 py-1">
                {proc.name} <span class="text-gray-400">#{proc.id}</span>
            </div>
        {/each}
    {:else}
        {#each pageState.result.data as node (node.process.id)}
